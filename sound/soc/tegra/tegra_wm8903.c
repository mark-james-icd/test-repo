...
static int tegra_hdmi_hw_params(struct snd_pcm_substream *substream,
					struct snd_pcm_hw_params *params)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_card *card = rtd->card;
	...
	i2s_daifmt = SND_SOC_DAIFMT_NB_NF;
	i2s_daifmt |= pdata->i2s_param[HDMI].is_i2s_master ?
			SND_SOC_DAIFMT_CBS_CFS : SND_SOC_DAIFMT_CBM_CFM;

	switch (pdata->i2s_param[HDMI].i2s_mode) {
		case TEGRA_DAIFMT_I2S :
			i2s_daifmt |= SND_SOC_DAIFMT_I2S;
			break;
		case TEGRA_DAIFMT_DSP_A :
...
static struct snd_soc_ops tegra_wm8903_hdmi_ops = {
...
printk("Creating mic jack object with gpio %d\n", tegra_wm8903_hp_jack_gpio.gpio);
		snd_soc_jack_new(codec, "Headphone Jack", SND_JACK_HEADSET,
		...
		machine->gpio_requested |= GPIO_HP_DET;
	        wm8903_mic_detect(codec, &tegra_wm8903_hp_jack, SND_JACK_MICROPHONE,
                          (machine_is_cardhu() || machine_is_houdini()) ? SND_JACK_MICROPHONE : 0);
	}
	...
	if (!machine_is_harmony() && !machine_is_ventana() &&
	    !machine_is_cardhu() && !machine_is_houdini())
...
static __devinit int tegra_wm8903_driver_probe(struct platform_device *pdev)
{
	...
	if (machine_is_cardhu() || machine_is_houdini() || machine_is_ventana()) {
	...
	if (machine_is_cardhu() || machine_is_houdini()) {
		...
		tegra_wm8903_dai[2].cpu_dai_name = "tegra30-i2s.0"; // for H2C on Houdini
		tegra_wm8903_dai[3].cpu_dai_name = "tegra30-i2s.2"; // for ext conn on Houdini
	}
	...
	} else if (machine_is_ventana() || machine_is_cardhu() || machine_is_houdini()) {
